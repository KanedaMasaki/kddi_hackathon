...
export const PictureComponent = () => {
  ...
  const albumBucketName = albumBucketName; //.envない
  const bucketRegion = bucketRegion; //.envない
  const IdentityPoolId = IdentityPoolId; //.envない
  ...
  return (
    <>
      ...
      <button className="btn btn-2 " onClick={handleClick}>
        再取得
      </button>
    </>
  );
